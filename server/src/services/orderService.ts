...
const FIND_BY_USER_ID =
  'SELECT REL.orderId, REL.lastUpdated, REL.userId, REL.count, P.* from product P, (select R.productId, R.count, O.orderId, O.lastUpdated, O.userId from orderProductRelation R, (select id as orderId, updatedAt as lastUpdated, userId from `order` where userId = :userId) O where O.orderId = R.orderId) REL where REL.productId = P.id;';
